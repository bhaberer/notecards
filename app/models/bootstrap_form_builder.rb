...
class BootstrapFormBuilder < ActionView::Helpers::FormBuilder
  ...
  %w[text_field text_area password_field select collection_select time_select time_zone_select
     email_field].each do |method_name|
    define_method(method_name) do |name, *args|
      errors = object.errors[name].any? ? ' error' : ''
      error_msg = if object.errors[name].any?
                    content_tag(:span, object.errors[name].join(','),
                                class: 'help-inline')
                  else
                    ''
                  end
  ...
  def field_label(name, *args)
    ...
    label(name, options[:label], class: "control-label#{required ? ' required' : ''}")
  end
